...
def parse_pagination_urls_of_city(city_default_url, city_url, offset, header):
    ...
    try:
        number_of_pages_in_city = soup.find(
            'a', attrs={'class': 'last'}).contents[0]
    except:
    ...
    for i in range(0, int(number_of_pages_in_city)):
        if i == 0:
            ...
            occurences_of_dash = [j for j in range(
                len(city_default_url)) if city_default_url.startswith('-', j)]
            ...
            current_city_pagination_url = city_default_url[:second_dash_index] + '-oa' + str(
                city_pagination) + city_default_url[second_dash_index:] + '#ACCOM_OVERVIEW'
...
def parse_hotel_urls_of_city(base_url, pagination_urls, header):
    ...
    for pagination_url in pagination_urls:
        ...
        for j, city_hotel_url in enumerate(soup.find_all('a', attrs={'class': 'property_title'})):
            hotel_urls.append(base_url + soup.find_all('a',
                                                       attrs={'class': 'property_title'})[j]['href'][1:])
            logger.info('PROCESSED: ' + base_url + soup.find_all('a',
                                                                 attrs={'class': 'property_title'})[j]['href'][1:])
    ...
    for hotel_url in hotel_urls:
        ...
        for i in range(0, maximum_pagination_of_hotel):
            if i == 0:
                ...
                # Calculate the dash positions
                occurrences_of_dash = [j for j in range(
                    len(hotel_url)) if hotel_url.startswith('-', j)]
                ...
                # Build the current page url and append it to the list
                hotel_page_url = hotel_url[:fourth_dash_index] + '-or' + str(
                    hotel_pagination) + hotel_url[fourth_dash_index:] + '#REVIEWS'
...
def parse_review_urls_of_hotel(base_url, pagination_urls, header):
    ...
    for pagination_url in pagination_urls:
        ...
        hotel_review_containers = soup.find_all(
            'div', attrs={'class': 'basic_review'})

        # Retrieve each review url of the current hotel pagination page
        for hotel_review_container in hotel_review_containers:
            quote = hotel_review_container.find(
                'div', attrs={'class': 'quote'})
...
def parse_reviews_of_city(review_urls, city_default_url, user_base_url, session_timestamp, header):
    ...
    city_directory_path = create_session_directory(
        city_default_url, session_timestamp)
    ...
    for i, review_url in enumerate(review_urls):
        logger.info('STARTED: Processing of ' + review_url +
                    ' (Review ' + str(i + 1) + ' of ' + str(len(review_urls)) + ')')

        # Calculate the dash positions
        occurrences_of_dash = [j for j in range(
            len(review_url)) if review_url.startswith('-', j)]

        # Get the hotel name out of the url
        hotel_name = review_url[occurrences_of_dash[3] +
                                1:occurrences_of_dash[4]].replace(' ', '_').lower()
        ...
        if hotel_name not in processed_hotels:
            try:
                ...
                hotel_directory_path = create_hotel_directory(
                    hotel_name, city_directory_path)
                rating_directory_paths = create_rating_directories(
                    hotel_directory_path)
                store_hotel_data_in_csv(
                    hotel_name, hotel_information, hotel_directory_path)
            except:
                logger.warning('WARNING: Processing of ' + review_url +
                               ' was skipped due to an unexpected error!')
                continue

        # Parse review information
        try:
            review_information = parse_review_information(
                review_url, user_base_url, header)
        except ValueError:
            logger.warning('WARNING: Processing of ' + review_url +
                           ' was skipped due to missing of essential information!')
            continue
        except:
            logger.warning('WARNING: Processing of ' + review_url +
                           ' was skipped due to an unexpected error!')
        ...
        try:
            store_review_data_in_csv(
                review_url, hotel_name, review_information, hotel_directory_path, headline_exists)
        except:
            logger.warning('WARNING: Processing of ' + review_url +
                           ' was skipped due to an unexpected error!')
            continue
        ...
        try:
            store_review_data_in_txt(
                review_url, rating_directory_paths, review_information)
        except:
            logger.warning('WARNING: Processing of ' + review_url +
                           ' was skipped due to an unexpected error!')
            continue

        logger.info('FINISHED: Processing of ' + review_url +
                    ' (Review ' + str(i + 1) + ' of ' + str(len(review_urls)) + ')')
...
def store_review_data_in_txt(review_url, rating_directory_paths, review_information):
    ...
    logger.info('STARTED: Storing of review text from ' + review_url + ' into ' + rating_path.replace(
        '\\\\?\\', '') + '\\review_' + review_url[occurences_of_dash[0] + 1:occurences_of_dash[3]] + '.txt')
    ...
    with open(rating_path + '\\review_' + review_url[occurences_of_dash[0] + 1:occurences_of_dash[3]] + '.txt', 'wb') as file:
        file.write(
            bytes(review_information[0]['text'], encoding='ascii', errors='ignore'))

    logger.info('FINISHED: Storing of review text from ' + review_url + ' into ' + rating_path.replace(
        '\\\\?\\', '') + '\\review_' + review_url[occurences_of_dash[0] + 1:occurences_of_dash[3]] + '.txt')
...
def store_review_data_in_csv(review_url, hotel_name, review_data, hotel_directory_path, headline_exists):
    logger.info('STARTED: Storing of review data from ' + review_url + ' into ' +
                hotel_directory_path.replace('\\\\?\\', '') + '\\' + hotel_name + '-reviews.csv')
    ...
    logger.info('FINISHED: Storing of review data from ' + review_url + ' into ' +
                hotel_directory_path.replace('\\\\?\\', '') + '\\' + hotel_name + '-reviews.csv')
...
def store_hotel_data_in_csv(hotel_name, hotel_data, hotel_directory_path):
    logger.info('STARTED: Storing of hotel data ' + hotel_name + ' into ' +
                hotel_directory_path.replace('\\\\?\\', '') + '\\' + hotel_name + '-information.csv')
    ...
    with open(hotel_directory_path + '\\' + hotel_name + '-information.csv', 'w') as csvfile:
        ...
        # Write headlines into the file
        csvwriter.writerow(['Name', 'Address', 'Description', 'Stars', 'Room Count', 'Amenities', 'TripAdvisor City Rank',
                            'Overall Rating', 'Review Count', 'Review Rating Count', 'Review Reason Count', 'Reviewer Languages'])
    ...
    logger.info('FINISHED: Storing of ' + hotel_name + ' into ' +
                hotel_directory_path.replace('\\\\?\\', '') + '\\' + hotel_name + '-information.csv')
...
def create_rating_directories(hotel_path):
    ...
    for star in stars:
        ...
        logger.info('STARTED: Creation of directory ' +
                    hotel_path.replace('\\\\?\\', '') + '\\' + str(star) + '-star')

        # Create the folder
        os.makedirs(directory_path)

        logger.info('FINISHED: Creation of directory ' +
                    hotel_path.replace('\\\\?\\', '') + '\\' + str(star) + '-star')
...
def create_hotel_directory(hotel_name, city_directory_name):
    ...
    logger.info('STARTED: Creation of directory ' +
                city_directory_name.replace('\\\\?\\', '') + '\\' + hotel_name)
...
def create_session_directory(city_default_url, session_timestamp):
    ...
    occurrences_of_dash = [j for j in range(
        len(city_default_url)) if city_default_url.startswith('-', j)]
    city_name = city_default_url[occurrences_of_dash[1] +
                                 1:occurrences_of_dash[2]].lower()

    # Build directory name
    directory_path = '\\\\?\\' + os.getcwd() + '\\data\\' + \
        session_timestamp + '-' + city_name

    logger.info('STARTED: Creation of directory ' + os.getcwd() +
                '\\data\\' + session_timestamp + '-' + city_name)

    # Create the folder
    os.makedirs(directory_path)

    logger.info('FINISHED: Creation of directory ' + os.getcwd() +
                '\\data\\' + session_timestamp + '-' + city_name)
...
def parse_hotel_information(review_url, header):
    ...
    hotel['name'] = soup.find(
        'a', attrs={'class': 'HEADING'}).text.replace('|', '').strip()
    hotel['overall-rating'] = soup.find(
        'img', attrs={'class': 'sprite-rating_no_fill'})['alt'][0:1] + ' stars'
    hotel['rank'] = soup.find(
        'div', attrs={'class': 'slim_ranking'}).text.strip()

    review_count = soup.find('h3', attrs={'class': 'reviews_header'}).text
    occurrences_of_spaces = [j for j in range(
        len(review_count)) if review_count.startswith(' ', j)]
    hotel['review-count'] = str(review_count[0:occurrences_of_spaces[0]])

    review_filter = soup.find(
        'fieldset', attrs={'class': 'review_filter_lodging'})

    hotel['address'] = soup.find(
        'span', attrs={'class': 'format_address'}).text.replace('|', '-')

    try:
        star_filter_items = review_filter.find(
            'div', attrs={'class': 'col2of2'}).find_all('div', attrs={'class': 'wrap'})
        star_filter_string = ''
        for star_filter_item in star_filter_items:
            description = star_filter_item.find(
                'span', attrs={'class': 'text'}).text
            count = star_filter_item.find(
                'span', attrs={'class': 'compositeCount'}).text
    ...
    try:
        reason_filter_items = review_filter.find(
            'div', attrs={'class': 'trip_type'}).find_all('div', attrs={'class': 'segment'})
        reason_filter_string = ''
        for reason_filter_item in reason_filter_items:
            description = reason_filter_item.find(
                'div', attrs={'class': 'filter_selection'}).text
            count = reason_filter_item.find(
    ...
    try:
        language_items = soup.find(
            'select', attrs={'id': 'filterLang'}).find_all('option')[:-1]
        languages = ''
    ...
    try:
        amenity_items = soup.find('div', attrs={'class': 'indent'}).find_all(
            'span', attrs={'class': 'amenity'})
        amenities = ''
    ...
    try:
        hotel['stars'] = str(soup.find('div', attrs={'class': 'stars'}).text.replace(
            'Hotel Class:', '').strip()[0:1])
    except:
        hotel['stars'] = 'n.a.'

    try:
        hotel['room-count'] = str(soup.find('span',
                                            attrs={'class': 'tabs_num_rooms'}).text.strip())
    except:
        hotel['room-count'] = 'n.a.'

    try:
        hotel['description'] = soup.find('span', attrs={'class': 'descriptive_text'}).text.strip(
        ) + soup.find('span', attrs={'class': 'descriptive_text_last'}).text.replace('|', '').strip()
    except:
...
def parse_review_information(review_url, user_base_url, header):
    ...
    # Parse review information
    try:
        review['title'] = entry_container.find('div', attrs={'class': 'quote'}).text.strip(
        ).replace('“', '').replace('”', '').replace('|', '')
        review['rating'] = entry_container.find(
            'img', attrs={'class': 'sprite-rating_s_fill'})['alt'][0:1] + ' stars'
        review['date'] = entry_container.find(
            'span', attrs={'class': 'ratingDate'})['content']
        review['text'] = entry_container.find('div', attrs={'class': 'entry'}).find(
            'p').text.replace('\n', ' ').replace('\r', '').replace('\t', '').replace('|', '').strip()
    except:
        logger.warning(
            'WARNING: Essential review information such as title or rating is missing!')
        raise ValueError(
            'Essential review information such as title or rating is missing!')

    try:
        stay = entry_container.find(
            'span', attrs={'class': 'recommend-titleInline'}).text
        occurences_of_colon = [j for j in range(
            len(stay)) if stay.startswith(',', j)]
        review['time'] = stay[0:occurences_of_colon[0]].replace('Stayed ', '')
        review['reason'] = stay[occurences_of_colon[0] +
                                2:].replace('traveled ', '')
    except:
        review['time'] = 'n.a.'
        review['reason'] = 'n.a.'
    ...
    try:
        review['helpful-votes'] = str(entry_container.find('span',
                                                           attrs={'class': 'numHlpIn'}).text)
    except:
        review['helpful-votes'] = 'n.a.'

    try:
        review['room-tip'] = entry_container.find('div', attrs={'class': 'inlineRoomTip'}).text.replace(
            'Room Tip: ', '').replace('See more room tips', '').replace('|', '').strip()
    except:
        review['room-tip'] = 'n.a.'
    ...
    try:
        ...
        recommendation_columns = entry_container.find('ul', attrs={'class': 'recommend'}).find(
            'li').find_all('ul', attrs={'class': 'recommend-column'})

        for column in recommendation_columns:
            recommend_answers = column.find_all(
                'li', attrs={'class': 'recommend-answer'})

            for answer in recommend_answers:
                recommend_description = answer.find(
                    'div', attrs={'class': 'recommend-description'}).text
                rating = answer.find(
                    'img', attrs={'class': 'sprite-rating_ss_fill'})['alt'][0:1] + ' stars'
    ...
    user_name = user_container.find('div', attrs={'class': 'username'}).find(
        'span', attrs={'class': 'scrname'}).text
...
def parse_reviewer_information(user_name, user_base_url, header):
    ...
    try:
        user['level'] = soup.find(
            'div', attrs={'class': 'level'}).find('span').text
    except:
        user['level'] = 'n.a'

    try:
        user['since'] = soup.find(
            'div', attrs={'class': 'ageSince'}).find_all('p')[0].text
    except:
        user['since'] = 'n.a.'

    try:
        user['demographic'] = soup.find(
            'div', attrs={'class': 'ageSince'}).find_all('p')[1].text
    except:
        user['demographic'] = 'n.a.'

    try:
        user['hometown'] = soup.find(
            'div', attrs={'class': 'hometown'}).find('p').text
    except:
        user['hometown'] = 'n.a.'

    try:
        number_of_reviews = soup.find_all(
            'a', attrs={'data-filter': 'REVIEWS_ALL'})[0].contents[0]
        occurences_of_space = [j for j in range(
            len(number_of_reviews)) if number_of_reviews.startswith(' ', j)]
    ...
    try:
        number_of_ratings = soup.find(
            'a', attrs={'data-filter': 'RATINGS_ALL'}).contents[0]
        occurences_of_space = [j for j in range(
            len(number_of_ratings)) if number_of_ratings.startswith(' ', j)]
        user['ratings'] = number_of_ratings[0:occurences_of_space[0]]
    except:
        user['ratings'] = 'n.a.'

    try:
        number_of_photos = soup.find(
            'a', attrs={'data-filter': 'PHOTOS_ALL'}).contents[0]
        occurences_of_space = [j for j in range(
            len(number_of_photos)) if number_of_photos.startswith(' ', j)]
        user['photos'] = number_of_photos[0:occurences_of_space[0]]
    except:
        user['photos'] = 'n.a.'

    try:
        number_of_helpful_votes = soup.find_all(
            'a', attrs={'data-filter': 'REVIEWS_ALL'})[1].contents[0]
        occurences_of_space = [j for j in range(
            len(number_of_helpful_votes)) if number_of_helpful_votes.startswith(' ', j)]
    ...
    try:
        tags = ''
        tag_bubbles = soup.find('div', attrs={'class': 'tagBlock'}).find_all(
            'div', attrs={'class': 'tagBubble'})
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='scrape the reviews of a whole city on tripadvisor',
                                     usage='python tripadvisor-scrapper 60763 New_York_City_New_York')
    parser.add_argument('id', help='the geolocation id of the city')
    parser.add_argument('name', help='the name of the city')
    parser.add_argument('--pickle', choices=['load', 'store'],
                        help='[load] store a scraped reviews list as pickle for later parsing,[load] load a scraped reviews list for parsing')
    parser.add_argument(
        '--filename', help='the filename of the pickle file placed in pickle directory')
    args = parser.parse_args()

    # Setup logger
    session_timestamp = time.strftime('%Y%m%d-%H%M%S')
    logging.basicConfig(filename='./logs/' + session_timestamp + '-' + args.name.lower() +
                        '.log', level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    ...
    if not args.pickle or args.pickle == 'store':
        logger.info('STARTED: Scraping of ' + args.name +
                    ' review urls. Build tree "city-pagination-urls--city-hotel-urls--hotel-pagination-urls--hotel-review-urls".')
        city_pagination_urls = parse_pagination_urls_of_city(
            CITY_DEFAULT_URL, CITY_URL, number_of_hotels_per_page, headers)
        city_hotel_urls = parse_hotel_urls_of_city(
            BASE_URL, city_pagination_urls, headers)
        hotel_pagination_urls = parse_pagination_urls_of_hotel(
            city_hotel_urls, headers)
        city_review_urls = parse_review_urls_of_hotel(
            BASE_URL, hotel_pagination_urls, headers)
        ...
        if args.pickle == 'store':
            ...
            with open(file_path, 'wb') as pickle_file:
                ...
                logger.info('STORED: Stored review urls list in ' +
                            os.getcwd() + '\\' + file_path)
        ...
        parse_reviews_of_city(city_review_urls, CITY_DEFAULT_URL,
                              USER_BASE_URL, session_timestamp, headers)
        ...
        with open('pickle/' + args.filename, 'rb') as pickle_file:
            ...
            logger.info('LOADED: Loaded review urls list from ' +
                        os.getcwd() + '\\pickle\\' + args.filename)
