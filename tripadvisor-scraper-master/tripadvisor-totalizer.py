...
def create_session_directory(directory_name):
    ...
    logger.info('STARTED: Creation of directory ' +
                os.getcwd() + '/totalized/' + directory_name)
...
def copy_review_files(source_directory, destination_directory):
    ...
    for file_name in src_files:
        ...
        if (os.path.isfile(full_file_name)):
            logger.info('STARTED: Copying ' + full_file_name +
                        ' to ' + destination_directory)
            shutil.copy(full_file_name, destination_directory)
            logger.info('FINISHED: Copying ' + full_file_name +
                        ' to ' + destination_directory)
...
def copy_review_csv_rows(source_directory, destination_directory):
    ...
    full_file_name = os.path.join(source_directory, file_name)
    logger.info('STARTED: Copying information from ' +
                full_file_name + ' to ' + destination_directory)
    if (os.path.isfile(full_file_name)):
        with open(full_file_name, 'r', encoding='ISO-8859-1') as src_csv:
            ...
            with open(destination_directory + '/reviews.csv', 'a', encoding='ISO-8859-1') as dest_csv:
                writer = csv.writer(dest_csv, delimiter='|',
    ...
    logger.info('FINISHED: Copying information from ' +
                full_file_name + ' to ' + destination_directory)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='put together all reviews of a city',
                                     usage='python tripadvisor-totalizer C:\\Users\\Administrator\\tripadvisor-scrapper\\2016-06-01-1522-vienna')
    parser.add_argument('path', help='path of city directory with reviews')
    args = parser.parse_args()

    # Setup logger
    session_timestamp = time.strftime('%Y%m%d-%H%M%S')
    logging.basicConfig(filename='./logs/' + session_timestamp + '-totalizer.log',
    ...
    for sub_directory_path in sub_directory_paths:
        sub_directory_name = os.path.basename(
            os.path.normpath(sub_directory_path))
